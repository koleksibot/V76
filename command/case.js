...
const { TiktokDownloader } = require('../lib/gif')
...
"inviteCode": "mememteeeekkeke","groupName": "Bot Indo", 
"caption": `Made With `, 'jpegThumbnail': thumbnail
}}}
		try {
			...
			if (anu.action == 'add') {
				...
				try {
					ppimg = await haruka.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				teks = `[ *WELCOME IN GC ${mdata.subject}* ] \n___________________________\n@${num.split('@')[0]} Intro/Dikick!!! \n Nama : \n Umur : \n Askot : \n Gender : \n Udah Punya Doi/Blm: \n Pap Muka dumlu!!! \n Instagram? \n  ! \n *___________________________*\n\nRead Deskription`
				deskripsi = `${mdata.desc}`
				gbutsan = [{buttonId:`#bebantot`,buttonText:{displayText:'Welcome beban gc '},type:1}]
				let buff = await getBuffer(ppimg)
				mhan = await haruka.prepareMessage(mdata.id, buff, MessageType.image, {thumbnail: buff})
				const buttonMessages = { imageMessage: mhan.message.imageMessage,
contentText: `${teks}`,
footerText: `${deskripsi}`, 
buttons: gbutsan,
headerType: 4 }
			haruka.sendMessage(mdata.id, buttonMessages, MessageType.buttonsMessage, {thumbnail: fs.readFileSync('./settings/logo.jpg'), "contextInfo": { mentionedJid: [num]}, caption: 'Tes', quoted: falfa}) 
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				try {
					ppimg = await haruka.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				teks = `SAYONARA... @${num.split('@')[0]}* \n_Jasamu akan saya kubur dalam dalam_`
				let buff = await getBuffer(ppimg)
				gbutsan = [{buttonId:'SERAH',buttonText:{displayText:'Byee, Tombol to isekai'},type:1}]
				mhan = await haruka.prepareMessage(mdata.id, buff, MessageType.image, {thumbnail: buff})
				const buttonMessages = { imageMessage: mhan.message.imageMessage,
contentText: `${teks}`,
footerText: `By Beban`,
...
shp = ''
var thu = await haruka.getStatus(anu.participants[0], MessageType.text)
num = anu.participants[0]
teks = `*P R O M O T E - D E T E C T E D*\n\n${shp} Username: @${num.split('@')[0]}\n\n${shp} Bio : ${thu.status}\n\n${shp} Group: ${mdata.subject}\n\nDon't break the rules!`
haruka.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}, quoted: falfa})
console.log(color('|TRM|'), color(`Promote Member ${num.split('@')[0]} In ${mdata.subject}`,  'cyan'))
} 
else if (anu.action == 'demote') {
shp = ''
thu = await haruka.getStatus(anu.participants[0], MessageType.text)
num = anu.participants[0]
...
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
				sendButLocation(from, lang.menu(prefix, salam, pushname), '© ' + ownername, thumbnail, [{buttonId: '.owner', buttonText: {displayText: 'Owner'}, type: 1},{buttonId: '.ping', buttonText: {displayText: 'Speed'}, type: 1},{buttonId: '.infobot', buttonText:{displayText: 'Infobot'}, type: 1}], {quoted: mek})
				break
case 'bebantot':
...
anu = await fetchJson(`https://api.lolhuman.xyz/api/checkapikey?apikey=${args.join(' ')}`)
...
case 'setppbot':
				if (!isOwner) return reply('*Only Owner bot*')
					...
					media = await haruka.downloadAndSaveMediaMessage(mek)
					await haruka.updateProfilePicture (from, media)
...

`
  sendButMessage(from, `LIST`, teksnya, [
            {
              buttonId: `#owner`,
              buttonText: {
                displayText: `OWNER`,
...
case 'owner':{
	if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
		const ownerContact = [ownernumber, "6281276698054", "6285797957303", "6281994112523", "628811929177", "", "", "", "", "", "" , "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""]
		let ini_list = []
		for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
			...
			ini_list.push({
				...
				hehe = await haruka.sendMessage(from, {
					...
					haruka.sendMessage(from, `Nih Kak Contact Owner Ku, Mereka Wibu🤝`, text, {quoted: hehe})
				}
			break
...
*WIKI :* ${res[0].wiki}`
sendMediaURL(res[0].thumb, result).catch(e => {
reply(result)
})
break
case 'donasi':
...
case 'blackpink':
                ...
                reply (lang.wait())
                    ...
                    ini_txt = args.join(" ")
                  buff = await getBuffer(`https://api.lolhuman.xyz/api/textprome/${command}?apikey=modal&text=${ini_txt}`)
                 haruka.sendMessage(from, buff, image,{quoted: fgif, caption: `nieh`})
                    break
case 'shadow':
                case 'cup':
                case 'cup1':
                ...
                reply (lang.wait())
                    if (args.length == 0) return reply(`Example: ${prefix + command} LoL Human`)
                    ini_txt = args.join(" ")
                  buff = await getBuffer(`https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=Modal&text=${ini_txt}`)
                          haruka.sendMessage(from, buff, image,{quoted: fgif, caption: `nieh`})
                    break
                case 'pornhub':
                case 'glitch':
                ...
                reply (lang.wait())
                    ...
                    txt1 = args[0]
                    txt2 = args[1]
                 buff = await getBuffer(`https://api.lolhuman.xyz/api/textprome2/${command}?apikey=modal&text1=${txt1}&text2=${txt2}`)
                          haruka.sendMessage(from, buff, image,{quoted: fgif, caption: `nieh`})
                    break
case 'tiktok':
                case 'arcade8bit':
                ...
                reply (lang.wait())
                    ...
                    txt1 = args[0]
                    txt2 = args[1]
                buff = await getBuffer(`https://api.lolhuman.xyz/api/photooxy2/${command}?apikey=modal&text1=${txt1}&text2=${txt2}`)
                      haruka.sendMessage(from, buff, image,{quoted: fgif, caption: `nieh`})
                    break
case 'restart':
             if (!isOwner) return 
...
haruka.sendMessage(from, tigabelas, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound14':
empatbelas = fs.readFileSync('./mp3/sound14.mp3');
haruka.sendMessage(from, empatbelas, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound15':
limabelas = fs.readFileSync('./mp3/sound15.mp3');
haruka.sendMessage(from, limabelas, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound16':
enambelas = fs.readFileSync('./mp3/sound16.mp3');
haruka.sendMessage(from, enambelas, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound17':
tujuhbelas = fs.readFileSync('./mp3/sound17.mp3');
haruka.sendMessage(from, tujuhbelas, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound118':
delapanbelas = fs.readFileSync('./mp3/sound18.mp3');
haruka.sendMessage(from, delapanbelas, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound19':
sembilanbelas = fs.readFileSync('./mp3/sound19.mp3');
haruka.sendMessage(from, sembilanbelas, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound20':
duapuluh = fs.readFileSync('./mp3/sound20.mp3');
haruka.sendMessage(from, duapuluh, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound21':
duasatu = fs.readFileSync('./mp3/sound21.mp3');
haruka.sendMessage(from, duasatu, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound22':
duadua = fs.readFileSync('./mp3/sound22.mp3');
haruka.sendMessage(from, duadua, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound23':
duatiga = fs.readFileSync('./mp3/sound23.mp3');
haruka.sendMessage(from, duatiga, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound24':
duaempat = fs.readFileSync('./mp3/sound24.mp3');
haruka.sendMessage(from, duaempat, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'sound25':
dualima = fs.readFileSync('./mp3/sound25.mp3');
haruka.sendMessage(from, dualima, MessageType.audio, {quoted: fgif, mimetype: 'audio/mp4', ptt:true})
break
case 'toptt':
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
...
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
  ...
  sendMediaURL(from,njir,`Nieh ${pushname}`)
  })
  break
case 'tiktokwm':
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
reply(lang.wait())
  ...
  .then((data) => { sendMediaURL(from, data.result.watermark, `Nieh`) })
...
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			if (!q) return reply('Linknya?')
			if (!isUrl(args[0]) && !args[0].includes('i.coco.fun')) return reply('Invalid Link')
			anu = await fetchJson (`https://bx-hunter.herokuapp.com/api/video-dl?url=${q}&apikey=FuckBitch`)
			buffer = await getBuffer (anu.thumb)
			haruka.sendMessage(from, buffer, image, {quoted: fgif, caption: anu.title})
			buffer = await getBuffer (anu.UrlMp4)
			haruka.sendMessage(from, buffer, video,{mimetype: 'video/mp4', quoted : fgif})
			break
...
case 'ytsearch':
             ...
             get_result = await fetchJson(`https://api.dhamzxploit.my.id/api/ytsearch?q=${query}`)
                    ...
                    reply(ini_txt)
                    break
case 'naruto': case 'rias': case 'akeno': case 'kaneki': case 'shinomiya': case 'kurama': case 'rimuru': case 'akira': case 'dazai': case 'miku': case 'mikasa': case 'eren': case 'titan': case 'levi': case 'sakura': case 'hinata': case 'neji': 
...
case 'ssweb':  
                   ...
                   reply(lang.wait())
                   anu = await getBuffer(`https://bx-hunter.herokuapp.com/api/ssweb?url=${args.join(' ')}&apikey=FuckBitch`)
